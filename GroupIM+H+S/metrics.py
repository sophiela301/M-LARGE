...
def ndcg_binary_at_k_batch_torch(X_pred, heldout_batch, k=100, device='cpu'):
    ...
    DCG = (heldout_batch_nonzero[torch.arange(
        batch_users, device=device).unsqueeze(1), idx_topk] * tp).sum(dim=1)
    # num. of non-zero items per batch. [B]
    heldout_nonzero = (heldout_batch > 0).sum(dim=1)
    IDCG = torch.tensor([(tp[:min(n, k)]).sum()
                        for n in heldout_nonzero]).to(device)
    return DCG / IDCG
...
def recall_at_k_batch_torch(X_pred, heldout_batch, k=100):
    ...
    _, topk_indices = torch.topk(X_pred, k, dim=1, sorted=False)  
    ...
    X_true_binary = (heldout_batch > 0).float()  # .toarray() #  
