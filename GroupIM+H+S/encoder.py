...
class Encoder(nn.Module):
    ...
    def __init__(self, n_items, user_layers, embedding_dim, drop_ratio):
        ...
        # user individual preference encoder layers.
        self.user_preference_encoder = torch.nn.ModuleList()
        ...
        # item embedding for pre-training
        self.user_predictor = nn.Linear(
    ...
    def pre_train_forward(self, user_items):
        ...
        for idx, _ in enumerate(range(len(self.user_preference_encoder))):
            user_pref_embedding = self.user_preference_encoder[idx](
                user_pref_embedding)  # [B, G, D] or [B, D]
            user_pref_embedding = torch.tanh(
                user_pref_embedding)  # [B, G, D] or [B, D]

        logits = self.user_predictor(
            user_pref_embedding)  # [B, G, D] or [B, D]
    ...
    def forward(self, user_items):
        ...
        user_embeds = torch.tanh(
            self.transform_layer(user_embeds))  # [B, G, D]
        return user_embeds
